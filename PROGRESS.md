...
- 类型检查：`tsc --noEmit` 通过
- 测试：116/116 全部通过（45 原有 + 17 邮箱 + 16 Dashboard EventBridge + 13 API + 11 AgentLoop + 6 邮箱跨实例 + 8 Metrics）
- 覆盖：
  ...
  - api-server（HTTP 端到端：健康检查、认证、任务管理、CORS、速率限制、body 限制）— 13 测试
  - agent-loop（mock 模型→工具执行→完成、预算/步数限制、权限/审批、生命周期回调）— 11 测试
  - mailbox-cross-instance（跨实例通信：点对点/广播/回复/waitFor/stats/多条消息）— 6 测试
  - metrics（Counter/Gauge/标签/导出/预设 Agent 指标）— 8 测试
...
- [ ] ~~Prometheus 指标~~ (已在 v0.6 实现)
- [ ] init 命令（脚手架生成 .agents/ + orchestrator.yaml + .env.example）
...
### v1.0 规划
1. **init 命令**: 脚手架生成 .agents/ + orchestrator.yaml + .env.example
2. **Dockerfile + CI/CD**: Docker 镜像 + GitHub Actions
3. **npm publish**: 全局安装 `npm install -g agent-orch`
4. **Dashboard 实际运行验证**: 用真实 Agent 任务测试 TUI 仪表盘

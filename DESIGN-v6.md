# Multi-Agent Orchestrator v6 — 轻量自编排架构
...
**v6 方案**：Agent 主循环内嵌复杂度判断，复杂任务通过 `task` 原语拆分子 Agent 会话。

### 1.2 对比

| 维度 | DESIGN-v4 | DESIGN-v6 |
|------|-----------|-----------|
...
interface AgentDefinition {
  ...
  model: string;               // 模型标识
  provider?: ModelProvider;    // 模型提供商（可选，未指定时由 AdapterSelector 决定）
...
class PermissionResolver {
   ...
   * 
   * 优先级规则：
   * 1. 全局配置 requireApproval 是最低安全基线
   * 2. Agent 级 tools 配置只能比全局更严格，不能更宽松
   * 3. 最终决策取两者中更严格的那个
   */
  canUse(
    ...
    // 取更严格的那个：deny > ask > allow
    const finalDecision = this.stricter(agentResult.decision, globalDecision);
  ...
  private resolveAgentLevel(
    ...
    if (toolName === "Bash" && bashCommand && typeof perm === "object") {
      return { decision: this.resolveBash(perm, bashCommand) };
    }
  ...
  private checkGlobalBaseline(toolName: string, bashCommand?: string): Permission {
    // 全局配置格式："file.delete", "git.push", "bash.exec" 等
  ...
  private stricter(a: Permission, b: Permission): Permission {
    const order = { deny: 3, ask: 2, allow: 1 };
    return order[a] >= order[b] ? a : b;
  }

  private resolveBash(perm: BashPermission, command: string): Permission {
    // deny 优先
...
class AgentLoop {
  ...
  async run(
    ...
    while (steps < definition.maxSteps) {
      ...
      // 2. 成本追踪（注意：token 计费模型下，单次调用可能突破预算，这是固有缺陷）
  ...
  private async spawnSubAgent(args: SubAgentArgs): Promise<SubAgentResult> {
    ...
    try {
      const subLoop = new AgentLoop(
        ...
        this.costTracker,  // 共享同一个 CostTracker，确保父子预算联通
        this.concurrencyLimiter
      );
      return await subLoop.run(args.task, subDef, this.costTracker.remaining);
    } finally {
      release();
    }
  }
...
/** task 工具 schema — 必须注册到 ChatParams.tools 中，模型才能调用 */
const taskTool: ToolDefinition = {
  name: "task",
  description: "Spawn a sub-agent to handle a subtask. Use this when the current task is complex and can be broken into smaller, independent pieces. The sub-agent runs in an isolated context and returns a summary of its work.",
  parameters: {
    type: "object",
    properties: {
      agentType: {
        type: "string",
        description: "The type of sub-agent to spawn, e.g. 'explore', 'coder', 'reviewer'",
        enum: ["explore", "coder", "reviewer"],  // 根据 .agents/ 目录动态生成
      },
      task: {
        type: "string",
...
class AdapterSelector {
  /**
   * 简陋但有效的模型选择：
   * - Agent 配置里指定了 provider 就沿用
   * - 未指定时默认 DeepSeek（能力更强），成本敏感时切 GLM
   */
...
> **已知限制**：token 计费模型下，单次模型调用的输出 token 数量无法预先精确知道。因此 `record()` 在调用后执行，可能出现"调用前预算未超、调用后超出"的情况。这是 LLM API 的固有特性，当前方案通过 80% 预警 + 步数上限双保险来缓解。

### 6.2 并发控制

```typescript
// src/agent/concurrency-limiter.ts
...
否 → deny     是 → 取值类型？
      ...
      deny  → 拒绝并记录
          │
          ▼
    再检查全局 requireApproval
    如果全局要求 ask，则强制提升为 ask
    （全局是最低安全基线，Agent 配置只能更严格）
```
...
security:
  ...
  requireApproval:
    - "file.delete"
    - "git.push"
    - "bash.exec"  # 所有 bash 命令默认需要批准（可被 Agent 配置覆盖为更严格）
...
│   │   └── tools.ts               # 工具定义（含 task 工具 schema）
│   ├── adapters/
...
│   ├── security/
│   │   ├── permission-resolver.ts # 权限解析引擎（glob 匹配 + 全局基线）
...
| 维度 | v5 | v6 |
|------|-----|-----|
| 成本追踪 | 父子 Agent 各用各的 CostTracker | **共享同一个 CostTracker**，预算真正联通 |
| task 工具 | 代码里处理了但无 schema 定义 | **补充完整 schema**，模型才能调用 |
| 权限控制 | 全局与 Agent 级冲突规则不明 | **明确规则**：全局是最低基线，Agent 只能更严格 |
| provider 字段 | 非可选，AdapterSelector 逻辑冗余 | **改为可选**，未指定时由 Selector 决定 |
| 流式响应 | MVP 声称支持但主循环未使用 | **明确 v0.2 实现**，MVP 只做非流式 |
| 并发控制 | 配置里有但代码中未体现 | **新增 ConcurrencyLimiter + Semaphore** |
| 预算突破风险 | 未说明 | **文档明确说明**是 token 计费固有特性 |
...
- 权限解析引擎（allow/ask/deny + Bash glob + 全局基线）
- 故障转移（同一模型重试 3 次 → 切备用模型）
- 成本双保险（budget + steps，父子共享 CostTracker）
- 并发控制（信号量限制 maxConcurrentAgents）
- 结构化日志
